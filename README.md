...
当前 PR-19 已在 PR-18 大画布工作区基础上补充正式 Demo 脚本。脚本优先覆盖稳定的语音主链路：基础绘图、颜色和位置控制、对象编辑、连续语音输入重开、删除、撤销、重做、清空、数量指令、房子模板拆解、AI 生图和语音改图；同时提供 LLM 语义容错可选片段和模型服务不稳定时的备用录制路线。
