...
| PR-18 | `feat/canvas-workspace-expansion` | 将固定三栏画布升级为轻量大画布工作区：扩大 SVG 世界坐标、浮动语音/日志面板、点阵画布背景和完整图片显示 | 单元测试覆盖大画布 viewBox 与图片显示策略，浏览器手动检查画布空间 |
| PR-19 | `docs/demo-readiness` | 完成 README、设计文档和 Demo 脚本 | 按 Demo 脚本完整跑通 |
| PR-20 | `fix/polish-and-hardening` | 修复最终演示问题和体验细节 | 全量回归测试 |
...
- 画布体验优先学习主流绘图工具的“大画布 + 视口 + 浮动工具面板”模式；侧栏不能长期挤压主要创作空间，日志和语音控制可以浮动或收起。
